...
router.get('/runs', async (req, res) => {
    try {
        const { projectKey, since, until, page, limit } = req.query;
        const project = projectKey ? await prisma.project.findUnique({ where: { key: projectKey } }) : null;
        let where = project ? { projectId: project.id } : {};
        // Add date filtering with validation
        if (since || until) {
...
router.post('/admin/db/preview', async (req, res) => {
    ...
    try {
        ...
        const serializedRows = JSON.parse(JSON.stringify(rows, (key, value) => typeof value === 'bigint' ? Number(value) : value));
...
router.post('/analytics/query', async (req, res) => {
    try {
        ...
        // Convert BigInt to regular numbers for JSON serialization
        const serializedResults = JSON.parse(JSON.stringify(results, (key, value) => typeof value === 'bigint' ? Number(value) : value));
...
router.get('/analytics/test/:testName/failures', async (req, res) => {
    try {
        ...
        const { projectKey, days = '90' } = req.query;
    ...
    `;
        ...
        // Convert BigInt to regular numbers for JSON serialization
        const serializedFailures = JSON.parse(JSON.stringify(failures, (key, value) => typeof value === 'bigint' ? Number(value) : value));
        res.json({
            testName,
...
router.get('/analytics/debug', async (req, res) => {
    ...
    `);
        // Convert BigInt to regular numbers for JSON serialization
        const serializedRecentCases = JSON.parse(JSON.stringify(recentCases, (key, value) => typeof value === 'bigint' ? Number(value) : value));
        const serializedFailingSummary = JSON.parse(JSON.stringify(failingSummary, (key, value) => typeof value === 'bigint' ? Number(value) : value));
        res.json({
            days: parseInt(days),
